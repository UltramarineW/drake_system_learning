...
'''
Change the order of state to [cart_x, cart_x_dot, pole_theta, pole_theta_dot]
'''
class StateDataBus(LeafSystem):
    def __init__(self):
        LeafSystem.__init__(self)
        ...
        def output(context, output):
            ...
            y = np.array([x[0], x[2], x[1], x[3]], dtype=float)
            output.set_value(y)
...
def create_scene(meshcat, model_location, sim_time_step):
    ...
    # constant_controller = builder.AddSystem(ConstantVectorSource(np.array([0, 0])))
    ...
    # ConstantController
    builder.Connect(
        constant_controller.get_output_port(),
        plant.get_actuation_input_port()
    )
    '''
    ...
    builder.Connect(
        plant.get_state_output_port(),
        lqr_controller.get_input_port()
    )
    builder.Connect(
        lqr_controller.get_output_port(),
        plant.get_actuation_input_port()
    )
